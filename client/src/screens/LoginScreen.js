...
import { auth } from '../../firebase'; 
import { useNavigation } from '@react-navigation/native';
import { login } from '../api/api'; // ✅ (Optional) to notify backend of login
...
export default function LogInScreen() {
  ...
  const handleLogin = async () => {
    try {
      ...
      // ✅ OPTIONAL: Inform backend of login to update lastLogin
      await login(user.email); // you can comment this if you don't want it
